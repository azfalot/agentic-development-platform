...
2. **AgentHub:** validates a contract, creates an isolated Git worktree, tracks state, and emits evidence for human review.
   Progress is contract-aware: an in-scope change is implementation unless it is classified as a test or documentation artifact. AgentHub never assumes that production code lives under one universal directory.
